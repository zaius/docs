...
function trackSignupClick (e) {
  ...
  return window.client.trackExternalLink(event, 'signup_click', {id: buttonId, page: pageId});
}
...
function trackCorporateClick (e) {
  var buttonId  =  e.target.getAttribute('id');
  var pageId    =  document.querySelector('body').getAttribute('id');
  return window.client.trackExternalLink(event, "corporate_click", {id: buttonId, page: pageId});
}
