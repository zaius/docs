...
Through the wercker.yml you are able to add notifications to your build and deploy pipelines on wercker. For instance, these notifications could send a message on a failed build or successful deploy.

Wercker has the notion of an additional phase in the wercker pipeline, after build and deploy steps, called `after-steps`, which can be used to execute steps after the build or deployment pipeline is finished. This makes them ideally suited for notifications.
...
[&lsaquo; After steps](/learn/steps/03_after-steps.html "nav previous steps")
[Creating steps &rsaquo;](/learn/steps/05_creating-steps.html "nav next steps")
